...
- Написать Денису: @denis_kodkontenta
- Сайт: kodkontenta.ru
...
3. Kod VPN — надёжный VPN-сервис (скоро)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const text = data.content?.[0]?.text || "Не могу ответить прямо сейчас. Напишите Денису в Telegram: @denis_kodkontenta";

    return NextResponse.json({ text });
  } catch {
    return NextResponse.json(
      { text: "Произошла ошибка. Напишите нам в Telegram: @denis_kodkontenta" },
      { status: 200 }
    );
  }
}
